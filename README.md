...
* 技能序列化 : 核心思路就是拆解，将技能拆为动画，位移，生成对象和子技能等事件，提取序列化信息，保存在预制体中。
* 技能释放 ：读取技能数据反序列化，得到技能事件队列，每帧检测事件队列，事件从start帧开始触发, 直到所有事件完成，技能便执行结束。
* 同步思路：
...
* 击退效果： 每个技能都配置了击退位移，浮空时间，顿帧时间。[思路](https://www.bilibili.com/read/cv17846274/?from=readlist)
* 敌人ai:抽象出敌人行为。配置成行为池,和抽卡一般,把行为按照概率随机抽空。
